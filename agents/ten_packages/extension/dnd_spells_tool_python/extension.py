...
SPELLS_TOOL_NAME = "get_spell_info"
SPELLS_TOOL_DESCRIPTION = "Get the description of a spell that the user has specified."
SPELLS_TOOL_PARAMETERS = {
        "type": "object",
        "properties": {
            "spell": {
                "type": "string",
...
class DndExtension(Extension):
    # api_key: str = ""
    tools: dict = {}

    def on_init(self, ten_env: TenEnv) -> None:
        logger.info("DndExtension on_init")
        ...
        self.tools = {
            ...
            # },
            SPELLS_TOOL_NAME: {
                TOOL_REGISTER_PROPERTY_NAME: SPELLS_TOOL_NAME,
                TOOL_REGISTER_PROPERTY_DESCRIPTON: SPELLS_TOOL_DESCRIPTION,
                TOOL_REGISTER_PROPERTY_PARAMETERS: SPELLS_TOOL_PARAMETERS,
                TOOL_CALLBACK: self._get_spell_info
            }
            # TODO other tools
        }
    ...
    def on_start(self, ten_env: TenEnv) -> None:
        logger.info("DndExtension on_start")
    ...
    def on_stop(self, ten_env: TenEnv) -> None:
        logger.info("DndExtension on_stop")

        ten_env.on_stop_done()

    def on_deinit(self, ten_env: TenEnv) -> None:
        logger.info("DndExtension on_deinit")
    ...
    def _get_spell_info(self, args:dict) -> Any:
        logger.info("USING DND THING")

        if "spell" not in args:
            raise Exception("Failed to get property")
        
        spell = args["spell"]
        url = f"https://www.dnd5eapi.co/api/{spell}"
